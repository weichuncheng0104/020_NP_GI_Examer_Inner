...
/* ── 目錄頁主題統計表（可排序，user 2026-06-15）──────────
   欄：主題｜通論+內(109–114)｜總題數｜內｜通論｜外。每欄 ▲升冪/▼降冪；
   數值欄次要鍵固定序 c109→total→im→common→surg（扣被選為主鍵者），一律降冪；主題欄依名稱。 */
const NUM_ORDER = ['c109', 'total', 'im', 'common', 'surg'];
const COLS = [
  {key:'name',   label:'主題'},
  {key:'c109',   label:'通論+內', sub:'109–114'},
  {key:'total',  label:'總題數'},
  {key:'im',     label:'內'},
  {key:'common', label:'通論'},
  {key:'surg',   label:'外'}
];
let SORT_KEY = 'c109', SORT_DIR = 'desc';   // 預設：109 通論+內 降冪

function tval(t, key){
  const c = t.counts || {};
  switch(key){
    case 'c109':   return t.c109 || 0;
    case 'total':  return c.total || 0;
    case 'im':     return c.inad_im || 0;
    case 'common': return c.common || 0;
    case 'surg':   return c.inad_surgery || 0;
    default:       return 0;
  }
}
function sortedTopics(){
  const arr = (MANIFEST.topics || []).slice();
  const s = SORT_DIR === 'asc' ? 1 : -1;
  if(SORT_KEY === 'name'){
    arr.sort((a,b) => s*String(a.name).localeCompare(String(b.name),'zh-Hant')
                      || String(a.slug).localeCompare(String(b.slug)));
    return arr;
  }
  const sec = NUM_ORDER.filter(k => k !== SORT_KEY);
  arr.sort((a,b) => {
    let d = s*(tval(a,SORT_KEY) - tval(b,SORT_KEY));
    if(d) return d;
    for(const k of sec){ d = tval(b,k) - tval(a,k); if(d) return d; }   // 次要鍵一律降冪
...
function renderTopicTable(){
  const head = '<tr>' + COLS.map(col => {
    const sub = col.sub ? `<span class="sub">${esc(col.sub)}</span>` : '';
    const on = d => (SORT_KEY === col.key && SORT_DIR === d) ? ' on' : '';
    return `<th>${esc(col.label)}${sub}<span class="sortbtns">` +
      `<button type="button" class="sb${on('asc')}" data-k="${esc(col.key)}" data-d="asc" aria-label="依此欄升冪">▲</button>` +
      `<button type="button" class="sb${on('desc')}" data-k="${esc(col.key)}" data-d="desc" aria-label="依此欄降冪">▼</button>` +
      `</span></th>`;
  }).join('') + '</tr>';
  const rows = sortedTopics().map(t => {       // manifest 經 GCM 認證，display 仍 escape 防殘留 XSS
    const c = t.counts || {};
    const sup = t.is_full_page ? '' : '<span class="sup">補充</span>';
    return '<tr>' +
      `<td class="tname"><a class="tlink" data-slug="${esc(t.slug)}">${esc(t.name)}</a>${sup}</td>` +
      `<td class="c109">${esc(t.c109 || 0)}</td>` +
      `<td>${esc(c.total || 0)}</td>` +
      `<td>${esc(c.inad_im || 0)}</td>` +
      `<td>${esc(c.common || 0)}</td>` +
      `<td>${esc(c.inad_surgery || 0)}</td>` +
    '</tr>';
  }).join('');
  $('topiclist').innerHTML = `<table class="ttab"><thead>${head}</thead><tbody>${rows}</tbody></table>`;
}
...
$('topiclist').addEventListener('click', e => {
  const sb = e.target.closest('.sortbtns button');           // 點欄標頭 ▲/▼ → 改排序鍵+方向、重繪表
  if(sb){ SORT_KEY = sb.dataset.k; SORT_DIR = sb.dataset.d; renderTopicTable(); return; }
  const link = e.target.closest('.tlink');                   // 點主題名 → 守衛化 openTopic
  if(link && link.dataset.slug) openTopic(link.dataset.slug);
});
